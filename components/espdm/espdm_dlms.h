...
static const int DLMS_LENGTH_OFFSET = 11; // Offset at which message length is stored
static const int DLMS_LENGTH_CORRECTION = 4; // Part of the header is included in the DLMS length field and needs to be removed

// Bytes after length may be shifted depending on length field

static const int DLMS_SECBYTE_OFFSET = 12; // Offset of the security byte

static const int DLMS_FRAMECOUNTER_OFFSET = 13; // Offset of the frame counter
static const int DLMS_FRAMECOUNTER_LENGTH = 4; // Length of the frame counter (always 4)

static const int DLMS_PAYLOAD_OFFSET = 17; // Offset at which the encrypted payload begins
